...
describe("createAnthropicProvider", () => {
  it("returns missing_api_key when the configured key is absent", async () => {
    const provider = createAnthropicProvider({ apiKeyEnvVar: "ANTHROPIC_API_KEY" });
  ...
  it("maps timeout-like provider failures to timeout", async () => {
    process.env.ANTHROPIC_API_KEY = "test-key";
    const create = vi.fn().mockRejectedValue(new Error("request timeout"));
    const provider = createAnthropicProvider({
      clientFactory: () =>
        ({
          messages: { create }
