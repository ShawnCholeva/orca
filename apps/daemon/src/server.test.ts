...
function seedEngineeringTemplate(db: ReturnType<typeof openDatabase>, now: () => string): void {
  ...
  const steps = [
    ...
    { id: 'prd', ordinal: 2, name: 'PRD / Destination', instructions: 'prd', outputSchema: stepOut, agentPreference: agentPref },
    ...
    { id: 'review', ordinal: 6, name: 'Fresh-Context Review', instructions: 'review', outputSchema: stepOut, agentPreference: agentPref },
...
describe('server routes', () => {
  ...
  it('POST /v1/goals returns 201 with a valid Goal payload', async () => {
    ...
    expect(body.goal.intent).toBe('desc');
...
describe('WebSocket /v1/events', () => {
  ...
  it('closing WS does not crash the server; events not delivered after close', async () => {
    ...
    await wsServer.inject({
      ...
      url: '/v1/goals',
      headers: { 'content-type': 'application/json', ...AUTH_HEADERS },
      payload: { title: 'after-close', intent: 'test intent' }
    });
...
describe('goal refinement and workspace routes', () => {
  ...
  it('POST /v1/goals/refine returns 200 with draft', async () => {
    const res = await server.inject({
      ...
      url: '/v1/goals/refine',
      headers: { 'content-type': 'application/json', ...AUTH_HEADERS },
      payload: { title: 'My Goal', intent: 'Goals:\n- ship it\nConstraints:\n- no budget' }
    });
    expect(res.statusCode).toBe(200);
...
describe('GET/PUT /v1/settings', () => {
  ...
  it('PUT /v1/settings unsupervised does not force-confirm an existing human_review goal\'s parked step', async () => {
    ...
    db.prepare(
      "INSERT INTO goals (id, title, intent, status, autonomy_level, created_at, updated_at, archived_at, orchestrator_provider, orchestrator_model, operating_mode) VALUES ('goal-hr', 'HR Goal', 'desc', 'active', 1, ?, ?, NULL, NULL, NULL, 'human_review')"
    ).run(now, now);
    db.prepare(
