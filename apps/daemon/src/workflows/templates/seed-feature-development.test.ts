...
describe("boot seed: both built-in templates are present", () => {
  it("both engineering and feature-development are seeded with is_built_in=1", () => {
    seedEngineeringTemplate(db, () => "2026-01-01T00:00:00.000Z");
    seedFeatureDevelopmentTemplate(db, () => "2026-01-01T00:00:00.000Z");

    const query = db.prepare("SELECT id, is_built_in FROM workflow_templates WHERE id IN (?, ?) ORDER BY id");
    const rows = query.all(ENGINEERING_ID, FEATURE_DEV_ID) as { id: string; is_built_in: number }[];
    ...
    expect(ids).toEqual([ENGINEERING_ID, FEATURE_DEV_ID].sort());
  });
});
