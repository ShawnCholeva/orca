...
const PROVIDER_ID: ModelProviderId = "orca/openai";
const ADAPTER_ID: AdapterId = "codex";
const PROPOSAL_START = "<ORCA_PROPOSAL>";
const PROPOSAL_END = "</ORCA_PROPOSAL>";

const READY_PATTERN =
  /\bcodex\s+(?:ready|interactive)\b|(?:^|\n)\s*(?:codex[>:]|>)\s*$/im;
const AUTH_PATTERN =
  /\blog(?:\s|-)?in\s+(?:required|needed)\b|\bauth(?:entication)?\s+(?:required|failed|expired)\b|\bnot\s+(?:signed|logged)\s+in\b|\bunauthorized\b/i;
const RATE_LIMIT_PATTERN =
  /\brate.?limit(?:ed|ing)?\b|\bquota\b|\btoo many requests\b|\binsufficient_quota\b|\b429\b/i;
const PERMISSION_PATTERN =
  /\bcommand approval required\b|\bpermission\b|\ballow\s+this\s+action\b|\brequires?\s+approval\b/i;
...
function buildRequestInput(request: OrchestrationRequest): string {
  return [
    ...
    "You are an Orca hidden orchestration worker using Codex.",
...
export const codexHiddenWorkerDriver: ProviderHiddenWorkerDriver = {
  ...
  summarizeDebug(output, fallback = "codex worker output unavailable") {
