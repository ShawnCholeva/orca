...
const PROVIDER_ID: ModelProviderId = "orca/google-gemini";
const ADAPTER_ID: AdapterId = "gemini-cli";
const PROPOSAL_START = "<ORCA_PROPOSAL>";
const PROPOSAL_END = "</ORCA_PROPOSAL>";

const READY_PATTERN =
  /\bgemini\s+(?:ready|interactive)\b|(?:^|\n)\s*(?:gemini[>:]|>)\s*$/im;
const AUTH_PATTERN =
  /\blog(?:\s|-)?in\s+(?:required|needed)\b|\bauth(?:entication)?\s+(?:required|failed|expired)\b|\bcredentials?\s+(?:missing|expired)\b|\bnot\s+(?:signed|logged)\s+in\b/i;
...
function buildRequestInput(request: OrchestrationRequest): string {
  return [
    ...
    "You are an Orca hidden orchestration worker using Gemini CLI.",
...
export const geminiHiddenWorkerDriver: ProviderHiddenWorkerDriver = {
  ...
  summarizeDebug(output, fallback = "gemini worker output unavailable") {
