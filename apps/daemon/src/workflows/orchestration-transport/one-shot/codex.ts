...
export type CodexOneShotResult =
  | {
      status: "proposed";
      parsed: unknown;
...
export interface CodexOneShotRunner {
  ...
  run(request: OrchestrationRequestT): Promise<CodexOneShotResult>;
}

export interface CodexOneShotRunResult {
  stdout: string;
  stderr: string;
...
export type CodexOneShotProcessRunner = (input: {
  command: string;
  args: string[];
  cwd: string;
  ...
  stdin: string;
  timeoutMs: number;
}) => Promise<CodexOneShotRunResult>;

export interface CreateCodexOneShotRunnerDeps {
  adapter: AgentAdapter;
  processRunner?: CodexOneShotProcessRunner;
  timeoutMs?: number;
...
export function createCodexOneShotRunner(deps: CreateCodexOneShotRunnerDeps): CodexOneShotRunner {
  const processRunner = deps.processRunner ?? runCodexProcess;
  ...
  return {
    adapterId: "codex",
    run: async (request) => {
      if (deps.adapter.id !== "codex") {
        return failed("one_shot_unavailable", "codex adapter strategy unavailable", null, 0);
      }
      ...
      if (!auth.ok || auth.authStatus !== "ready") {
        return failed("one_shot_unavailable", "codex adapter is not ready", null, 0);
      }
      ...
      try {
        ...
        return failed(
          ...
          error instanceof Error ? error.message : "codex spawn resolution failed",
          null,
          0
        );
      }
      ...
      const stdin = buildCodexInput(parsedRequest.kind, requestJson);
      ...
      if (runResult.timedOut) {
        return failed("one_shot_unavailable", "codex one-shot timed out", null, runResult.latencyMs);
      }
      ...
      if (runResult.exitCode !== 0) {
        if (RATE_LIMIT_PATTERN.test(combined)) {
          return failed(
            ...
            "codex reported rate or quota limits",
...
function buildCodexInput(kind: OrchestrationRequestT["kind"], requestJson: string): string {
  return [
...
function sanitizeFailureMessage(message: string): string {
  ...
  return bounded.length > 0 ? bounded : "codex one-shot failed";
}
...
async function runCodexProcess(input: {
  ...
  return await new Promise<CodexOneShotRunResult>((resolve) => {
