...
export function composeOrchestratorPrompt(input: OrchestratorPromptInput): OrchestratorPrompt {
  const systemPrompt = [
    ...
    '- {"kind":"approve_step_complete","scoring":{"reasoning":"<work through the evidence>","successScore":0.0,"quality":{"outputCompleteness":0.0,"outputCorrectness":0.0,"instructionAdherence":0.0,"downstreamReadiness":0.0,"riskLevel":0.0},"reason":"<short>","handoffReady":true}}  (the agent\'s orca:step-complete block satisfies the step; ALWAYS include scoring when you approve)',
    ...
    '- {"kind":"stop_run","body":"<brief acknowledgment and where things stand>"}  (the user asked for the work to stop, pause, halt, or hold — this ACTUALLY stops the run and shuts the agent down)',
    ...
    'When the user asks you to stop, pause, halt, hold off, or otherwise call off the work, return stop_run. This is the ONLY way anything actually stops: answer_user_directly can write "stopping here" but changes nothing, and a run left going after the user asked it to stop will keep restarting itself. Never claim in prose that you have stopped, halted, or held anything — if you mean it, return stop_run and let the harness say so.',
