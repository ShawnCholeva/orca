...
- **Solved bucket (regression check):** outputs whose `step_complete` transition has `RefuteFacet.verdict === "upheld"` (primary), OR — when no refute ran — `EvidenceFacet.verdict === "passed"` (fallback). The **step-level independent verdict is authoritative**: a refute facet present but not `upheld` (refuted/uncertain/unavailable, incl. a corrupt/unparseable blob) is *not* solved and never falls through to the evidence check. (Run-level terminal status is deliberately **not** an additional filter — for a per-step counterfactual, whether *this step's* output was independently verified is the correct anti-circularity signal; a later downstream run failure does not unmark a step whose own output passed. As-shipped: `learning/corpus.ts buildSolved`.) These are the previously-solved cases the edit must not break.
