...
- **Label weights:** a **refute** label weighs **1.0** (a direct adversarial verdict on *this* completion), a **vindication** label weighs `vindicatorWeight(...)` ∈ (0,1] (an *indirect* downstream proxy, attenuated by the vindicator's strength). This asymmetry is intentional — direct vs. indirect evidence.
- **Effective sample size = the SUM of label weights** (not the count), and `sampleSize` in `CalibrationEntry` is that weighted sum. This is the anti-gaming property (paper §5.2.2 — "the harness should know when a signal is strong enough to act on"): a source vindicated only by *weak* vindicators (e.g. shadow gates at 0.4) needs ~2.5× as many labels to reach the `CALIBRATION_SCORE_MIN` (10) score-application threshold as one anchored by strong verifiers — so a chain of weak checks cannot cheaply move a score.
...
- [ ] **Step 1: failing tests** — cover: (a) `independent_review` now calibrates from vindication labels (e.g. bucket of gate-approved completions mostly bounced → posterior mean < 0.55); (b) executable/grounding combine refute + weighted vindication; (c) no-vindication + no-refute → posterior mean == prior (`measured` may be null/insufficient at low n); (d) `self_report` unmeasurable. Assert posterior means with `toBeCloseTo`. Plus two guard tests:
  - **CRUX:** `independent_review`'s posterior is unaffected by refute verdicts — a bucket of refute-upheld-but-downstream-bounced `independent_review` completions must move its posterior *down* (from the bounces) and NOT up from the upholds (refute never labels `independent_review`). Assert the posterior tracks vindication only.
  - **Anti-gaming (weighted-n):** a source labeled only by shadow-gate vindications (weight 0.4) reaches `state: "measured"`/score-application only after ~2.5× the labels a worker-gate-labeled (0.55) or human-anchored (1.0) source needs — assert `sampleSize` (weighted sum) crosses the threshold at the expected weighted count, not the raw count.
- [ ] **Step 2: RED.**
...
- [ ] **Step 3: implement:**
  ...
  3. `usecases.ts` `getTemplateMetricsDetail`: compute the vindication map + gate-approval predicate **before** calibration, and pass `computeCalibration(transitions, { vindication, graph, gateApprovedByCompletion })`. Also **split version-excluded from pending**: change `vindicationByCompletion`'s return type to `VindicationOutcome | "excluded" | undefined` — a version-mismatched completion returns `"excluded"` (was: fell through to `pending`). In `aggregate.ts`'s tally, `if (o === "excluded") continue;` (skip — counts toward no bucket), so `pending` now means only "latest-version completion, downstream hasn't ruled yet". `undefined` (predicate not supplied) still omits the field entirely.
...
This phase makes a measured signal *load-bearing on the score* — the paper (§3.5 AHE, "governed promotion") treats revising the harness substrate as something that must be **bounded, inspectable, and non-behavior-gating**. This design satisfies all three:

| Decision | Paper prescription |
|----------|--------------------|
| Beta calibration + per-source sample counts surfaced (`calibrationMix`) | p.65 / §5.2.2: *"each feedback signal should expose its **scope and uncertainty** … treat feedback as **calibrated evidence, not a binary success signal**."* p.12: KnowNo — *uncertainty-aware* control. |
| Weighted effective-n threshold (`CALIBRATION_SCORE_MIN`) before a measured rate moves the score | §5.2.2: *"the harness should know **when a signal is strong enough to act on**, when it is weak, and when additional evidence is required."* |
| Prior-weighted attenuation so weak vindicators contribute weak labels | §5.2.2: *"if the verifier is weak, the agent will learn to **optimize against the wrong signal**."* |
| Calibration **informs but never gates** — the score is advisory; human-authoritative completion is untouched; `executable` caps *down* only | §3.5 "governed promotion"; p.64: high-stakes decisions stay HITL. The score is not high-stakes (advisory), so auto-calibration is the *bounded, non-gating* form. |
| Recompute-on-read, append-only spine, inspectable calibration readout | §3.5.1 deep telemetry as the optimization substrate; "auditable state transitions." |

**Governance stance (bounded, inspectable, non-gating):** the calibrated confidence is **advisory** — it informs the Metrics tab and (later) autonomy readiness, but never gates a human's "done" decision. It is **bounded** (executable capped-down, weighted-n threshold, prior shrinkage — a single anomalous window can't swing a coefficient far). It is **inspectable** (`calibrationMix` exposes assumed vs. measured vs. sample size per source, so a human can audit *why* a score moved). Do not add any path where the calibrated score gates advancement, completion, or a permission decision — that would cross into the HITL-mandatory territory the paper reserves for governed, auditable approvals.
