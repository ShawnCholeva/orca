...
- 🟡 **Ledger commit + Gate / splitter routing — ONE bidirectionally-coupled cluster, NOT two independent seams** (the original framing was wrong). `commitAdvanceOrComplete` (ledger) calls `parkForGateApproval`/`evaluateAndParkSplitter` (routing) *and* recurses into `requestNextDecision`; `routeGateDestination` ⇄ `evaluateAndParkSplitter` mutually recurse and call back into `spawnStepAgent`/`requestNextDecision`. The pure ledger-write trio (`completeStepWithLedger`, `commitStepOutputAndLedger`, `createStepOutputArtifact`) is cleanly separable, but the advance/route core re-enters the dispatch loop and can't be cleanly split without a forced interface that may harm more than help. **Deferred to its own brainstorm** — re-decide scope (pure ledger trio only vs. an advance/route engine with a fallback) before acting.
...
- *Follow-up note:* `ProviderRecoveryController` currently imports `readGoal`/`preferencesForGoal` (runtime fns) from `service.ts` — clean one-directional seam (no cycle), but a small shared-types/helpers module would let `service.ts` be a consumer too; do it when the C/D cluster is tackled.
