...
- 🟡 **Per-step / per-workflow budget hard-stop (external-eval gap, 2026-06-28).** A runaway step has no spend floor: the guardrail enum (`packages/contracts/src/workflows/index.ts:237-245`) has no budget kind, `cost_speed_preference` always returns `allow` and only ranks operators (`workflows/guardrails/evaluator.ts:216-222`), and the pre-dispatch guardrail pass gates only on concurrency/risk/approval (`dispatch-engine.ts:748-760`) — a step burns until it finishes or crashes. This is a **permission-tier / policy-gateway** gap in the paper's sense (actions constrained by "policies and permission tiers"; the PEV loop's *permissioned state transition*), and it bears on FUTURE_ARCHITECTURE's cost spine — subscription-billing economics (§2) and **server-side L5 autonomy** ("the laptop need not be open"), where an unbounded autonomous step is exactly the unsafe case. Per-step OTEL cost capture is already live for Claude (Phase 3 / `harness-telemetry/`), so the spend signal exists now. **Resume:** add a `budget_rule` guardrail kind with a per-step-type / per-workflow cap, evaluated in the **control-plane** pre-dispatch pass (deterministic core owns the gate); on over-budget, deny with a structured `budget_exhausted` outcome the engine routes on — escalate to the human under `human_review`, hard-stop the run under `automated`. Spend totals arrive via the Runner→server telemetry stream (the same OTEL hook events), so the decision stays control-plane and needs no execution-plane filesystem access. Does **not** gate the Phase 1 exit criterion below — additive Governed-axis hardening. Goal-total visibility lands with the Phase 2 cost roll-up; per-step caps don't need it.
...
> The Stateful axis is **live** — belief-divergence fires from a launch-vs-complete workspace version snapshot, the conflict policy derives from `operating_mode` (auto under Autonomous, escalate under Supervised), multi-workspace read/write-sets key off `session.workspace_id`, and `mark_done` carries a cumulative write-set roll-up (see `ORCA.md` §14). These are the remaining follow-ups.

- 🟡 **2.7 Goal-total cost roll-up on `mark_done`.** The cumulative **write-set** roll-up already lands; the **cost** twin doesn't — `mark_done` still hardcodes `cost: null` (`recommendations/usecases.ts:553-558`). **UNBLOCKED since 2026-06-28** — the OTEL Claude smoke made per-step `cost.usd` trustworthy. **Resume:** add a `buildGoalCostRollup(db, goalId, workflowRunId)` summing each `step_complete` transition's `telemetry_json.cost` (tokens in/out, cache tokens, usd) into a `CostEntry`, populated onto the mark_done `TelemetryFacet.cost` alongside the existing write-set roll-up at the same site. No migration/contract change; TDD offline. Not built yet by user choice.
- 🟡 **2.8 Assumption-level claim verification on corrections (fabrication rollback — external-eval gap, 2026-06-28).** A revision that invents new structured claims — file paths that don't exist, APIs that aren't real — passes through today if the orchestrator-LLM finds it plausible (searched: no snapshot→diff→reject of a correction anywhere). The only path-existence check is advisory and Done-step-only (`workflows/orchestrator/service.ts:1733-1769`: `existsSync` against the goal's workspace roots, never blocks). This is precisely the paper's *"synchronize artifacts but not assumptions"* gap (p.64): an artifact asserting a nonexistent path is an **assumption-level inconsistency**, and the fix is a **verifier obligation** on the write-set's claim-set, not just a file diff. It lands directly on the Stateful-axis substrate already built — the StateDepsFacet read/write-sets, version-deps, and the per-step `workspace_id` resolution from **2.3** — making it a follow-on, not a new axis. Especially dangerous cross-workspace: an `account-api` agent can invent paths in `idme-idp` that the next step acts on. **Resume:** snapshot the artifact's structured claim-set before a correction, diff after, and reject/re-revise if the correction introduced new file/API references that don't resolve against the step's `session.workspace_id` root(s) — generalize the Done-step `existsSync` check, risk-gate it via the existing classifier (not universal), and surface the violation as a structured issue (feeds 5.3). **Plane-split note:** resolving a claim against the working tree reads real repo state → it is an **execution-plane** capability (behind `RunnerPort`), with the reject/rollback **decision** staying control-plane; don't bake a control-plane filesystem assumption into it (the current Done-step `existsSync` runs in-daemon only because the planes are still fused).

> **Exit criterion:** belief-divergence fires from live workspace state on a **real run**, and the conflict WARN/auto path is reachable end-to-end. The code + unit/integration coverage landed (the WARN/auto half is reachable through the real completion flow + the `operating-mode` route); the **live-daemon smoke** is the open item — rebuild/restart the daemon on this branch, run a real goal, mutate the workspace mid-step, confirm `belief_divergence` on `/harness-metrics`.
...
- 🟡 **OTEL Codex half.** Claude is live-confirmed end-to-end (`ORCA.md` §14); **Codex's struct-form `[otel.exporter."otlp-http"]` branch remains unexercised** — the one real gap left in the OTEL smoke. Note: `provider_id` is always null (distinguish providers by `model` prefix); Codex `mcp-server` spawn has no OTEL (interactive/`exec` only); antigravity has no usage channel → always `cost=null`. Needs a live Codex run to close the two-provider exit clause.
...
The genuine gaps from this eval were folded into the phases: **Phase 1** budget hard-stop (permission-tier / cost-spine), **2.8** assumption-level claim verification (fabrication rollback, Stateful axis), **5.3** issue-list correction, **5.4** advisor framing.
