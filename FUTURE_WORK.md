...
### 0.1 — Harness substrate factory: facet / boundary / sensor registries

The single highest-leverage item. Consolidate the now-stable substrate behind three self-registering registries (a closed, runtime-enumerable vocabulary, via a `def*`-macro-over-registry pattern). Goal: *LLM-as-compiler + enforced consistency* — narrow the substrate so generated/edited code can't structurally drift. Scope confirmed as **all three registries**.

- 🟡 **`defFacet` registry** (the clear win). One spec per facet `{ key, column, schema }` as the single source for what is currently synced by hand across `packages/contracts/src/harness/index.ts:188` (facet fields of `HarnessTransition`), `apps/daemon/src/harness-transitions/usecases.ts:24` (`RecordTransitionInput`), and `apps/daemon/src/harness-transitions/projection.ts` (`COLS` :17, `INSERT` :31, `insert.run` args :70, `rowToTransition` :55). Adding/changing a facet becomes one registry entry + one migration column. Use **registry-derived runtime wiring + a load-time/test conformance guard** (registry keys == contract facet keys == projection columns); deliberately *not* mapped-type contract generation (rejected as too clever per ORCA.md §11 simplicity doctrine).
- 🟡 **`defBoundary` emit factory.** Close the `HarnessTransitionBoundary` vocabulary (`index.ts:3`) and declare which facets each boundary carries; generate typed emitters that become the **only sanctioned write path**, replacing the 5 string-literal `recordHarnessTransition` sites (`workflows/orchestrator/service.ts:1767,1846,2498,3352`, `apps/daemon/src/permission-gate.ts:34`). **Validate on write** (this is also the fix for the read-path-validates/write-path-doesn't gap — `recordHarnessTransition` currently returns the in-memory row unvalidated) and **wire the dormant `mark_done` boundary**. *This is the same set of emit sites as the OrchestratorService "transition emit" slice in 0.2 — do them as one effort.*
- 🟡 **`defSensor` registry.** Generalize `LABEL_TO_SCRIPT` (`apps/daemon/src/harness-sensors/detect.ts:14`) into a registry keyed by `WorkflowSensorKind` (`index.ts:11`), with a conformance check that every enum kind is registered or explicitly unimplemented (fixes the `integration`/`static` declared-but-dead drift). **Natural home for the deferred Executable-axis sensor work in Phase 3** (lint/static sensors, per-ecosystem resolvers).
- 🟡 **Optional: `GET /v1/harness/registry` introspection route** exposing boundaries + facet contracts + sensors, so an agent can enumerate the substrate at runtime. *(This is the concrete deliverable behind the Phase 0 exit criterion "runtime-enumerable.")*
- **Resume:** write the spec + implementation plan in `docs/superpowers/` (periodically cleaned, not permanent) when picked up.

### 0.2 — Decompose `OrchestratorService` (~4,645 lines)

🟡 The entire mediation engine — dispatch, provider recovery, judgement, scoring, ledger commit, gate/splitter routing — lives in one class (`workflows/orchestrator/service.ts`, ~482–4645). It is the most safety-critical module in the system yet the largest and hardest to review; a 4.6k-line file is where latent bugs hide, and at L4/L5 those execute *past* a human gate. **No behavior change** — extract along the natural seams already present, as collaborator modules the class delegates to, **one seam per PR**, leaning on the existing orchestrator test suite as the behavior-preservation guard:

- **Provider recovery** state machine (`loadProviderRecoveryContext`, `waitForProvider`, `retryProvider`, `refreshProviderRecovery`, `switchProvider`, `startRecoveryReplacementSession`, ~1079–1413).
- **Step scoring & result building** (`scoringFacts`, `buildApprovalStepResult`, `withResultSummary`, `replayEvaluationFailedResult`, `scoreCompletedStepResult`, ~2899–3097).
- **Ledger commit** (`completeStepWithLedger`, `commitStepOutputAndLedger`, `createStepOutputArtifact`, `commitAdvanceOrComplete`, ~3484–3764).
- **Gate / splitter routing** (`parkForGateApproval`, `buildSplitEvaluationRequest`, `evaluateAndParkSplitter`, `decideGate`, `routeGateDestination`, ~3800–4645).
- **Harness transition emit sites** (`recordStepLaunchTransition` + the `recordHarnessTransition` calls at ~1767, 1846, 2498, 3352) — do this slice **via the `defBoundary` emit factory in 0.1**; they target the same sites, so the two efforts are one.
