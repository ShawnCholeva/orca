...
| **1** | Close the Governed axis | No ungated worker path exists; supervision settings behave correctly per-goal. **Budget hard-stop ✅ (2026-06-30);** antigravity gate is the one open item. |
| **2** ✅ | Activate the Stateful axis | Belief-divergence is live end-to-end; the conflict WARN/auto path is reachable. **— COMPLETE (2026-06-30):** confirmed live on a real run (and fixed a baseline-recording bug found by the smoke); 2.7 + 2.8 landed. |
| **3** | Complete Executable & Inspectable | Full sensor ladder; OTEL verified on a live run; replay is correct. **Claude OTEL re-confirmed live; Codex half is the one open clause.** |
| **4** | Surfaces & features on the harness | The conversational/adapter/template/workspace surfaces are caught up to the substrate. **Honest instructions + workspace picker ✅ (2026-06-30);** other surfaces remain. |
...
The harness Governed axis is complete *except* for the antigravity gate below — a genuine safety hole. (The per-step/per-workflow **budget hard-stop** landed 2026-06-30 — a `budget_rule` guardrail kind evaluated in the control-plane pre-dispatch pass, routing an over-budget deny by `operating_mode`: escalate under `human_review`, hard-stop under `automated`. It ships **inert** — no built-in template carries a cap yet, by design — until a cap is authored. Durable shape folds into `ORCA.md` §14.)
...
> **Exit criterion:** no worker provider spawns ungated; the global supervision flip and new-goal default both respect per-goal `operating_mode`. **(Antigravity is the one path still ungated; budget hard-stop landed 2026-06-30 — see intro.)**

---

# Phase 2 — Activate the Stateful axis — ✅ COMPLETE (2026-06-30)

> The Stateful axis is **live and end-to-end confirmed** — belief-divergence fires from a launch-vs-complete workspace version snapshot, the conflict policy derives from `operating_mode` (auto under Autonomous, escalate under Supervised), multi-workspace read/write-sets key off `session.workspace_id`, and `mark_done` carries cumulative write-set **and cost** roll-ups (see `ORCA.md` §14). The follow-ups (2.7, 2.8) landed and the exit-criterion live smoke passed. Durable facts fold into `ORCA.md` §14.

Done in this phase (durable shape in `ORCA.md` §14):

- **2.7 Goal-total cost roll-up on `mark_done`** — `buildGoalCostRollup` sums each `step_complete` transition's `telemetry.cost` (tokens, cache, usd) into a `CostEntry` on the `mark_done` `TelemetryFacet.cost`, alongside the write-set roll-up. Returns null when no step reported a cost (antigravity/pre-OTEL stays null). The learning-loop training signal FUTURE_ARCHITECTURE §Layer 2 wants.
- **2.8 Assumption-level claim verification on corrections (fabrication rollback)** — on a correction, the corrected output's **new** file-path claims are diffed against the prior attempt and any that don't resolve against the step's `session.workspace_id` root are rejected with a bounded "fix only these" re-revise (the paper's PEV routing). The verified-to-exist claims are recorded as **scoped `StateAssumption` evidence-bundle entries** (`verified:true`, statement declares existence-only scope — paper p.62/p.64). Plane-split honored: fs resolution is injected (execution-plane-movable behind `RunnerPort`); the reject decision stays control-plane. Migration `0048` snapshots the prior claim-set. *Gating note:* it's gated correction-only + new-claim-only (non-universal) rather than the originally-planned permission-classifier, because `decideGate`'s polarity would weaken enforcement under autonomy — the paper backs the verification-axis framing over the permission-tier one.
- **Belief-divergence baseline fix (the smoke caught a real bug)** — `spawnStepAgent` (the path that launches the **first step** of every run plus every advance/gate-routed step) never emitted a `step_launch` transition; only `commitAgentStepDecision`'s direct-launch path did, which the bootstrap/advance flow doesn't use. So no step captured a launch-time workspace version and belief-divergence could never fire on a real run. Fixed by mirroring `recordStepLaunchTransition` into `spawnStepAgent`; **live-confirmed end-to-end** (real goal, mutate mid-step → `belief_divergence` on `/harness-metrics`, `state_consistency` 0.5).

> **Exit criterion — MET (2026-06-30):** belief-divergence fired from live workspace state on a real run (UI goal → mutate workspace mid-step → `belief_divergence` recorded on the `step_complete` transition and reflected on `/harness-metrics`); the conflict WARN/auto path is reachable end-to-end.
...
- ✅ **Honest/participatory instructions for the secondary templates — DONE (2026-06-30).** `orca/code-review`, `orca/refactor`, and `orca/quality-coverage` were rewritten to the participatory voice the adaptive-delivery / bug-triage templates already use (pause-and-ask at user-owned forks, treat prior step output as untrusted evidence, YAGNI on change steps, closing summary instead of finishing silently); each bumped to version 3. (Feature / Initiative are subsumed into `orca/adaptive-delivery`; Bug was already rewritten — see Appendix A.)

## Workspaces

- ✅ **Goal-creation workspace picker — DONE (2026-06-30).** The `2026-06-21` spec is implemented: a "Pick from registered" affordance beside "Browse…" opens a `WorkspacePickerModal` that lists registered workspaces and, on select, hydrates through the same `inspect → inspectSucceeded` path the filesystem browse uses (source-agnostic `pendingWorkspaces`); the empty state offers a jump to the Workspaces tab (the resolved "land on the tab" default). Validated live via Playwright against the daemon. *(Note: in plain-browser mode Tauri's folder dialog is unavailable, so the picker is also the only add-workspace path there — register via the API/Workspaces tab first.)*
