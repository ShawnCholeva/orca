...
- 🟡 **DispatchEngine split (the principled larger move).** Relocate the whole cohesive engine (`requestNextDecision` + `commitAdvanceOrComplete` + gate/splitter routing + `spawnStepAgent` + `advanceToNextStep` + `reviseStep` + decision/event plumbing) into a `DispatchEngine` class, leaving `OrchestratorService` as the event-handler/reaction layer (`onWorkflowSessionCompleted`/`onSessionOutputChunk`/`onAgentResponseDone`/`onUserMessage`/`confirmStep`/revisions/…). Verified the handler→engine dependency is **one-directional/acyclic** (no engine method calls back into a handler), so unlike the piecemeal extraction this is a *clean* split — but a large, high-risk relocation of the most safety-critical code with a wide seam. **Its own brainstorm/spec/SDD cycle**, not piecemeal.
