...
| # | Finding | Status |
|---|---|---|
| H1 | **36 minutes of real agent work left no trace at all**, on the founder's most expensive run. `execution` (step run `c7e08096`, one attempt, 03:42:14→05:37:16) shows a **47-minute gap with zero harness transitions** between 04:32:18 and 05:19:23. The activity store shows why: a `tool_use` activity created 04:32:18 and expired 05:08:11 — the agent was using tools continuously, and **none of those calls required approval, so none emitted a `tool_gate`.** Verified it is neither an orchestration stall (the agent was working) nor a park (status `expired`, never `paused_for_input`). This is the concrete cost of only two hooks being wired, and the strongest case for `PostToolUse`. | OPEN — measured |
