...
| A8 | **The run is parked on the user and the UI renders no control to unpark it.** `activities` has `mark_done_pending` / `paused_for_input` since 04:48:40 (with `step_confirmation_pending` expired and `step_result` written in the same 8ms bracket), while `workflow_step_runs` still says `status=active`, `orchestrator_phase='independent_check'`, `awaiting_user=0`, with `finished_at` already written. The ledger reads `activities` and says "mark done · waiting on you"; the chat reads the step run and says "Running an independent check…" — **so the mark-done card never renders.** A strictly worse variant of A2: not "Orca went quiet", but "Orca is waiting on you and won't show you the button". Suspect a non-atomic write across the two stores, `awaiting_user` derived from phase rather than from the activity, and stale in-memory phase state after the 04:42:54 restart. | **OPEN — live reproduction** | Caught only by two surfaces reading two tables and disagreeing. |
| A8-orig | Orchestrator stuck in `orchestrator_phase='independent_check'`. Refute completed (valid JSON verdict, pane idle, `Cooked for 30s · done`), `finished_at` written, but status stayed `active` and the phase never advanced. `pending_completion_json` NULL. Preceded by a daemon restart mid-flight (`crash_retries=2`). Suspected restart-durability gap in the completion path — the worker survives now, the orchestrator's memory of what it was doing does not. | **OPEN — live reproduction** | Run `01a06587`. `finished_at` set while `status='active'` should not coexist. |
